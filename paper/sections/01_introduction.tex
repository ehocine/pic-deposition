...
This paper reports CPU microbenchmarks on Apple M1 Pro, GPU atomic deposition on Tesla T4, and shared physics validation.
...
CUDA deposition kernels (atomics and multi-block privatized tiles) are benchmarked on Tesla T4 alongside CPU results on Apple M1 Pro.
...
\begin{itemize}
    ...
    \item A systematic comparison of four deposition schemes with charge, energy, spectral-noise, and long-run conservation diagnostics.
    \item Cache-aware CPU study separating sort cost from deposit-kernel throughput for AoS/SoA layouts.
    \item Langmuir-wave validation, OpenMP privatization on Apple Silicon, and roofline-style GPU/CPU performance analysis.
\end{itemize}
